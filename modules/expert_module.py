...
def write_audit_log(entry: dict):
    """FR-5.1: Immutable, timestamped audit log in JSONL format."""
...
app = FastAPI(
    ...
    description=(
        ...
        "Evaluates AI agents for safety, governance, and UN principle alignment using "
        f"Llama 3.2 via Ollama. Module ID: {MODULE_ID}"
    ),
    version=VERSION,
)
...
def parse_llama_response(raw_text: str) -> dict:
    ...
    # Derive confidence score from verdict and harm
    confidence_map = {
        ("REJECT", "high"):   0.92,
        ("REJECT", "medium"): 0.85,
        ("FLAG",   "medium"): 0.78,
    ...
    # Reasoning trace = full model output (trimmed)
...
async def evaluate(request: EvaluationRequest):
    """
    FR-4.1: Core evaluation endpoint.
    Submits a prompt to Llama 3.2 via Ollama and returns a structured
    safety assessment with verdict, confidence score, and reasoning trace.
    FR-5.1: All evaluation events are written to the audit log.
    """
    ...
    # FR-5.1: Write to immutable audit log
    audit_entry = {
        "event":          "evaluation",
        "timestamp":      timestamp,
        "session_id":     session_id,
    ...
    logger.info(f"Evaluation {evaluation_id} | {parsed['verdict']} | {parsed['confidence_score']:.2f} | {latency_ms:.0f}ms")
...
async def get_audit_log(limit: int = 50):
    """
    FR-5.1: Return recent audit log entries in JSON format.
    FR-5.2: Audit log supports reproducibility verification.
    """
...
async def module_info():
    ...
    return {
        ...
        "input_schema":  {"prompt": "str", "context": "str|null", "session_id": "str|null"},
        "output_schema": {"response_text": "str", "verdict": "ACCEPT|FLAG|REJECT",
                          "confidence_score": "float", "reasoning_trace": "str",
